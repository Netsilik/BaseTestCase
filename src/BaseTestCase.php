...
abstract class BaseTestCase extends phpUnitTestCase
{
	...
	/**
	 * Since PHPUnit does, unfortunately, NOT expose the configuration setting, we *assume* this value
	 * @var bool $_convertNoticesToExceptions
	 */
	protected $_convertNoticesToExceptions = false;
	
	/**
	 * Since PHPUnit does, unfortunately, NOT expose the configuration setting, we *assume* this value
	 * @var bool $_convertWarningsToExceptions
	 */
	protected $_convertWarningsToExceptions = false;
	
	/**
	 * Since PHPUnit does, unfortunately, NOT expose the configuration setting, we *assume* this value
	 * @var bool $_convertErrorsToExceptions
	 */
	protected $_convertErrorsToExceptions = true;
	...
	private function _setupErrorHandler() : void
	{
		...
		$errorTypes = 0;
		if (!$this->_convertNoticesToExceptions) {
			$errorTypes |= E_NOTICE | E_USER_NOTICE | E_STRICT;
		}
		if (!$this->_convertWarningsToExceptions) {
